...
public:
    int minimumSum(vector<vector<int>>& A) {
        ...
        for (int rot = 0; rot < 4; rot++) {
            ...
            for (int i = 1; i < n; i++) {
                // Top part
                vector<vector<int>> topPart(A.begin(), A.begin() + i);
                int a1 = minimumArea(topPart);

                // Case 1: Split bottom into left & right
                for (int j = 1; j < m; j++) {
                ...
                // Case 2: Split bottom into top & bottom again
...
private:
    int minimumArea(const vector<vector<int>>& A) {
        ...
        if (right == -1) return 0; // no '1' found
